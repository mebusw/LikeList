...
<%
    ...
    int out_trade_no = AttrParser.getParaInt(request, "out_trade_no");//订单号
    String body = AttrParser.getParaStr(request, "body");
	...
	if(mysign.equals(sign) && responseTxt.equals("true")){//验证成功
		...
		if(trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")){
            ...
            EsjOrderInfo order = tradeDao.findOrder(out_trade_no);
            if (null != order && !order.getPaid()) {
                order.setPaid(true);
                order.setBuyerAliAccount(buyer_email);
                tradeDao.updateOrder(order);
                EsjShop shop = shopBizDao.findShop(order.getShopId());
                Calendar calendar = Calendar.getInstance();
                calendar.setTime(shop.getDealPublishExpire());
                calendar.add(Calendar.DAY_OF_YEAR, 365);
                shopBizDao.renewDealPublishExpire(order.getShopId(), calendar.getTime());
            }
