	...
	if(bid > 0) {
		...
		while (iter.hasNext()) {
			...
			if (!item.isFormField()) {
			    ...
			    generatedName = "d" + dealId + "_" + (new Date().getTime()) + "." + ext;
            ...
            else if (item.getFieldName().equals("dealId")) {
                dealId = item.getString();
                //System.out.println("shopId=" + shopId);
            }
		}
	    ...
	    response.sendRedirect("../biz/uploadDealAvatar.jsp?step=2&picUrl=" + generatedName
	    		+ "&shopId=" + shopId + "&dealId=" + dealId);
