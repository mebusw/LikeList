...
public class ImageHepler {
	...
	private static void saveSubImage(BufferedImage image,
		...
		if (subImageBounds.width > image.getWidth()
			...
			g.setColor(Color.white);
			g.fillRect(0, 0, subImageBounds.width, subImageBounds.height);
			Image cutImage = image.getSubimage(subImageBounds.x, subImageBounds.y,
					subImageBounds.width, subImageBounds.height);
			g.drawImage(cutImage, 0, 0, null);
	...
	public static void cut(File srcImageFile, File descDir, int width,
		...
		if(rect.x<0)
			rect.x=0;
		else if(rect.width> (width-rect.x))
			rect.width = width-rect.x;
		if(rect.y<0)
			rect.y=0;
		else if(rect.height> (width-rect.y))
			rect.height = width-rect.y;
